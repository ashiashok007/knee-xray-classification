...
# ======== Data Generators ========
# Fix 1: Use consistent preprocessing across all datasets
...
checkpoint = ModelCheckpoint(
    os.path.join(models_dir, 'densenet121_checkpoint_sat_2.h5'),
    monitor='val_accuracy',
    save_best_only=True,
    verbose=1
)
...
# ======== Stage 2: Fine-tune the last dense block ========
print("Stage 2: Fine-tuning the last dense block...")
# Get the index of the last dense block
for i, layer in enumerate(base_model.layers):
    if 'conv5_block' in layer.name:
        last_dense_block_idx = i
        break

# Freeze all layers except the last dense block
for layer in base_model.layers[:last_dense_block_idx]:
...
# Get the index of the fourth dense block
for i, layer in enumerate(base_model.layers):
    if 'conv4_block' in layer.name:
        fourth_dense_block_idx = i
        break

# Freeze all layers except the last two dense blocks
for layer in base_model.layers[:fourth_dense_block_idx]:
    layer.trainable = False
for layer in base_model.layers[fourth_dense_block_idx:]:
    layer.trainable = True
...
# ======== Save Final Model ========
model_save_path = os.path.join(models_dir, "improved_densenet121_knee_classification_sat_2.h5")
model.save(model_save_path)
print(f" Improved DenseNet121 saved at: {model_save_path}")
...
np.save(os.path.join(models_dir, "improved_densenet_preds_sat_2.npy"), predictions)

# ======== Classification Report ========
report = classification_report(y_true, y_pred, target_names=labels, output_dict=True)
report_df = pd.DataFrame(report).transpose()
report_df.to_csv(os.path.join(models_dir, "classification_report_improved_densenet_sat_2.csv"))
...
plt.ylabel("True")
plt.tight_layout()
plt.savefig(os.path.join(models_dir, "confusion_matrix_improved_densenet_sat_2.png"), dpi=300)
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join(models_dir, "roc_curve_improved_densenet_sat_2.png"), dpi=300)
plt.close()
...
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(os.path.join(models_dir, "accuracy_plot_improved_densenet_sat_2.png"), dpi=300)
plt.close()
...
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(os.path.join(models_dir, "loss_plot_improved_densenet_sat_2.png"), dpi=300)
plt.close()
